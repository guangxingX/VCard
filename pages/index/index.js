...
Page({
    data: {
        btnarr: [{
            icon: "/assets/images/index/icon/1.png",
            name: "        找投资        ",
            url: "/pages/financial-index/index",
            currenttab: 0,
            page: 'financial-index'
        }, {
            icon: "/assets/images/index/icon/2.png",
            name: "        找项目        ",
            url: "/pages/financial-index/index",
            currenttab: 1,
            page: 'financial-index'
        }, {
            icon: "/assets/images/index/icon/3.png",
            name: "商协会广场",
            url: "../maillist/maillistIndex/maillist",
            currenttab: 1,
            page: 'maillistpage'
        }, {
            icon: "/assets/images/index/icon/4.png",
            name: "         活动         ",
            url: "/pages/activity/activityIndex/activity",
            currenttab: 1,
            page: 'activity'
        }],
        businessCard: [],
    ...
    jumpTo(e) {
        if(e.currentTarget.dataset.page == 'financial-index')
            app.currenttab = e.currentTarget.dataset.currenttab
        ...
        // console.log('存的啥')
        if(e.currentTarget.dataset.page == 'activity'){
            wx.navigateTo({
                        ...
                        if (that.data.companyCard.length > 0 && that.data.companyCard[0] && that.data.businessCard.length > 0) {
                            ...
                            for (var i = 0; i < that.data.companyCard.length; i++) {
                                if (that.data.businessCard[that.data.swiperCurrent] && that.data.businessCard[that.data.swiperCurrent].companyId == that.data.companyCard[i].id) {
                                    console.log("jinruif");
                                    that.setData({
                                        companyCardInfo: that.data.companyCard[i],
                                        companyCardInfoshow: true
                                    });
                                    return false;
                                } else {
                                    console.log("else");
