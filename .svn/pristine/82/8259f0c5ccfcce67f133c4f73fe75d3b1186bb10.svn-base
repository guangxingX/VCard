...
Page({
    ...
    goAddressDes: function (e) {
        ...
        // wx.navigateTo({
        //   url:'/pages/mine/companyAddressMaps/companyAddressMaps?address='+address
        // })
    },
    makeCall: function (e) {
        app.makeCall(e)
    ...
    toMycardDetails: function (e) {
        // console.log('去名片详情')
        // console.log(this.data.collectionList1);
        // console.log(e.currentTarget.dataset.mycardid);
        const mycardid = e.currentTarget.dataset.mycardid
        var name,companyName,companyId;
        this.data.collectionList1.forEach(item => {
            if (item.id == mycardid) {
                name = item.name
        ...
        info.legalPerson = name

        //把信息存入共同对象里
        prevPage.setData({
            info,
            wissoCardId:mycardid,
            companyId:companyId,
        })
        wx.navigateBack()
        // wx.navigateTo({
