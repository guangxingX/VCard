// pages/mine/editEveryCompanyProduct/editEveryCompanyProduct.js
...
Page({
  ...
  getdata(){
    var that = this;
    wx.request({
      url: app.data.apiurl+'applets/productInfo',
      data: {
      ...
      success: function (res) {
        ...
        if (res.statusCode == 200) {
          if (res.data.success == 0) {
            that.setData({ImageTextItem:res.data.entity.productInfo,
              productName:res.data.entity.name,
              productUrl:res.data.entity.url});
  ...
  savedata(){
    ...
    if(this.data.productName==''){
      this.setData({errorshow:true,failuretip:'请填写产品名称'});
    }else if(that.data.ImageTextItem.length==0){
      this.setData({errorshow:true,failuretip:'产品简介不能为空'});
    }else {
      console.log("进入提交数据00000000000");
      wx.request({
        url: app.data.apiurl+'applets/updateProduct',
        data: {
          id: that.data.productId,
          name: that.data.productName,
          url:that.data.productUrl,
          productInfo: JSON.stringify(that.data.ImageTextItem),
  ...
  getPositionDomByXY:function (potions) {
    console.log("************"+this.data.scrollPosition.top)
      var y = potions.y-this.data.scrollPosition.top+this.data.scrollPosition.scrollTop-45;
      ...
      for(var i=0,j=ImageTextItem.length;i<j;i++){
          if(y>=i*everyOptionCell&&y<(i+1)*everyOptionCell){
            console.log(y)
            console.log((i+1)*everyOptionCell)
    ...
    scrollTouchStart:function (event) {
        ...
        //movable-area滑块位置处理
        var movableX = 0;
        var movableY = firstTouchPosition.y-this.data.scrollPosition.top-this.data.scrollPosition.everyOptionCell/2-45;
        this.setData({
            movableViewPosition:{
                x:movableX,
                y:movableY,
    ...
    scrollTouchMove:function (event) {
        ...
        //movable-area滑块位置处理
        var movableX = 0;
        var movableY = event.changedTouches[0].pageY-this.data.scrollPosition.top-this.data.scrollPosition.everyOptionCell/2-45;
        this.setData({
            movableViewPosition:{
                x:movableX,
                y:movableY,
  ...
  onLoad: function (options) {
    ...
    this.getuptoken();
      ...
      var scrollViewHeight = systemInfo.windowHeight*(1-246/603);
